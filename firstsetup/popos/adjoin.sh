...
sudo apt-get install acl attr krb5-user samba samba-common-bin sssd sssd-tools libnss-sss libpam-sss realmd adcli policykit-1 packagekit samba-vfs-modules samba-libs samba-dsdb-modules ssh winbind -y

#Add the domain name to the ntp servers list
echo "Adding 'server ${LCDOMAIN}' to /etc/chrony.conf"
sudo mv /etc/chrony.conf /etc/chrony.conf.bak
echo "server ${LCDOMAIN} iburst" > /etc/chrony.conf
